...
# - Agent: spawns optional VoltAgent specialist agents for second-opinion
#   verification of P0/P1 edits (Phases 4, 7, 8; capped at 3 spawns/run).
# - Read/Grep/Glob: gathering review reports, plan documents, and domain
#   context (Phase 1); extracting and cross-referencing findings (Phases 3-4).
# - Write: applying edits to the plan document (Phase 7) and appending to
#   integration_log.jsonl (Phase 11). No Bash usage is described anywhere in
#   this skill's instructions, so it is intentionally not declared here.
---
