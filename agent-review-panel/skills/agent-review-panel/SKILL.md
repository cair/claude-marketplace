...
# Vendoring note (marketplace fix, additive only — no instruction text changed):
# - Agent: spawns the independent reviewer subagents (Phase 3-7, 12b, 13) and
#   the Supreme Judge (Phase 14) / Post-Judge Verification (Phase 14.5).
# - Read/Grep/Glob: context gathering (Phase 1), source verification (Phases
#   9-11, 13), and reading state files written by subagents.
# - Bash: upstream instructions already restrict this to a read-only
#   verification allowlist (grep/cat/head/tail/wc/git show/log/rev-parse) for
#   Phase 9 command execution and Phase 1 codebase-state checks. Declaring it
#   here does not grant any capability beyond what the prompt text already
#   describes — no new allowance introduced.
# - Write: final report generation only (Phase 15.1/15.2/15.3 output files).
---
