...
## Usage
Invoke via the orchestrating skill or directly as a subagent. Provide the PDF path and labels:
...
Note: this agent reads the PDF and source `.tex` files to reason about layout — it does not compile or execute anything. If the PDF has not been built yet, ask the user (or the `latex-figure-specialist` agent, which does have build tooling) to compile it first.
